...
(one universal hemi-rhombic-dodecahedron interface + shape-specific
adapters — triangle/square/pentagon/golden-rhombus/two kite variants/
a regular hexagon) letting any two Platonic/Archimedean/Catalan solids
bolt together through a shared, standardized joint. Normative geometric
spec: **`docs/RVCMG.md`** (James Baker's own writeup). Full scoping,
status, and postmortem in **`docs/rvcmg-adapter-pieces-spec.md`** (own
doc, not duplicated here, matching how Catalan solids/prisms-
...
divide-only design was recognized as incomplete — proven both ways via
`split-demos/{heptagon,octagon}.ts`. **All 6 originally-planned physical
