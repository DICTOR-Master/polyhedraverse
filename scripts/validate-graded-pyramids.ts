 ...
 * -- grade 1 exactly reproduces the existing D4, height increases
 * monotonically as apex angle decreases, and the degenerate limit
 * (360/n degrees) is real and correctly rejected, not just asserted.
 */
...
const grade1 = buildGradedPyramid(3, 60, 'TRI_PYR_G1', 'x');
const d4 = POLYHEDRA.D4;
check('grade 1 (triangular base) exactly reproduces D4 (regular tetrahedron)', grade1.vertices.every((v, i) => dist(v, d4.vertices[i]) < 1e-9));

const heights = DEFAULT_GRADES.map((g) => apexHeightForAngle(3, g.apexAngleDeg));
check('height increases monotonically as apex angle decreases (grade 0 -> 3)', heights[0] < heights[1] && heights[1] < heights[2] && heights[2] < heights[3]);
