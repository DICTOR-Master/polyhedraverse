import { derivePentagonToRdH } from './pentagonToRdH';
...
const result = derivePentagonToRdH();
...
check('separating the single step (with the real inverse deformation) reproduces the original hemi-RD hex interface exactly', statesApproximatelyEqual(back, before));
