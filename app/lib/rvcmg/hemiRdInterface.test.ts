...
// Confirms the interface's real edge-length pattern computationally
// rather than assuming it -- assert the actual measured spread (4 short
// + 2 long opposite edges), so a future accidental simplification of
// this construction fails loudly.
const sorted = [...edgeLengths].sort((a, b) => a - b);
check('at least two distinct edge lengths present (the real RD cross-section)', sorted[0] < sorted[5] - 1e-6);
check('exactly 4 short + 2 long edges', Math.abs(sorted[3] - sorted[0]) < 1e-6 && Math.abs(sorted[5] - sorted[4]) < 1e-6 && sorted[4] - sorted[3] > 1e-6);
